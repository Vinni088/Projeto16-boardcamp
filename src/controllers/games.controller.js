...
export async function insertRental(req, res) {
    ...
    try {
        ...
        let rentalsById = await db.query(` SELECT * FROM rentals WHERE "gameId" = $1`, [gameId] )
        ...
        await db.query(string, [customerId, gameId, daysRented, rentDate, originalPrice]);
...
export async function finishRental(req, res) {
    ...
    try {
        let rentalsById = await db.query(` SELECT *, TO_CHAR(rentals."rentDate", 'YYYY-MM-DD') AS "rentDate" FROM rentals WHERE id = $1`, [id] );
        let game = await db.query(` SELECT * FROM games WHERE id = $1`, [rentalsById.rows[0].gameId])
        ...
        let diferença = (dataAtual.getTime() - dataAluguel.getTime())/(1000*60*60*24);
        if (diferença > rentalsById.rows[0].daysRented) {
            delayFee = (diferença - rentalsById.rows[0].daysRented)*game.rows[0].pricePerDay;
        }
        await db.query(`
...
export async function deleteRental(req, res) {
    ...
    try {
        ...
        if (rental.rows[0].returnDate === null) {
            return res.status(400).send("O aluguel selecionado ainda não foi finalizado")
        }
